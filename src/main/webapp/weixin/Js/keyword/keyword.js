   ...
   function doSearch() {
            ...
            $('#cusTable').bootstrapTable({
                url: basePath+'/pc/getKeywordList.do',         //请求后台的URL（*）
                method: 'get',                      //请求方式（*）
    ...
    function toUpdate(id) {
    	window.location.href=basePath + "/pc/toGetUpdateWxKeyword.do?id="+id;
    }

    function toAdd() {
    	window.location.href=basePath + "/pc/toGetAddWxKeyword.do";
    }
    ...
    function toBatchDelete() {
    	...
    	if($data.length>0){
    		...
    		$.ajax({
    			type : "POST",
    			url : basePath + "/pc/batchDeleteKeyword.do",
    			data : {
    				"ids" : ids,
    			},
    			async : false,
