...
/*
@Override
public List<User> getUserList(String level, String username,
	RowBounds rowBounds) {
List<User> userList = userMapper.getUserListByLevel(level, username);
// com.github.pagehelper.PageInfo<User> uInfo=new
// com.github.pagehelper.PageInfo<User>(userList);
...
/*
@Override
public void updateUserDetails(String userId, UserEditDetails userEditDetails) {
List<Integer> roleIdList = userRoleMapper.getRoleIdListByUserId(userId);
User userTempUser = userEditDetails.getUser();
User user = null; // userMapper.selectByPrimaryKey(userTempUser.getId());
if (user == null) {
 ...
 * for (Integer roleId : roleIdIntegers) { if
 * (!roleIdListPre.contains(roleId)) { roleIdAdds.add(roleId); }
 * } for (Integer roleId : roleIdListPre) { if
 * (!roleIdIntegers.contains(roleId)) {
 * roleIdDeletes.add(roleId); } }
		 
		// 批量增加
		if (!roleIdAdds.isEmpty()) {
			
 * userRoleMapper.addUserRoleList(userTempUser.getId(),
 * roleIdAdds);
			 
		}
		if (!roleIdDeletes.isEmpty()) {
			
 * userRoleMapper.deleteUserRoleList(userTempUser.getId(),
 * roleIdDeletes);
	...
	if (userTempUser.getName() != null
			&& !userTempUser.getName().equals(user.getName())) {
		// 确保唯一性
...
if (!StringUtil.isEmpty(userTempUser.getPassword())) {
	if (!user.getPassword().equals(
			MD5Util.getMD5(userTempUser.getPassword().getBytes()))) {
		user.setPassword(MD5Util.getMD5(userTempUser.getPassword()
				.getBytes()));
	}
 ...
 */
/*
 * @Override public void createUser(String username, String password, String
 * des, String tel, String address) { if (userMapper.getUserNameCount(username)
 * > 0) { throw new MyException("该用户名已经存在"); } User user = new User();
 * user.setName(username);
 * user.setPassword(MD5Util.getMD5(password.getBytes())); user.setTel(tel);
 * user.setDes(des); user.setLevel(1); user.setAddress(address);
 * userMapper.insertSelective(user); }
 * 
 * @Override public List<Role> getRoles(String id) {
 * 
 * return roleMapper.getRoleList(id); }
 * 
 * @Override public void addRole(String roleName) { if
 * (roleMapper.getNameCount(roleName) > 0) { throw new MyException("该角色名已经存在");
 * } Role role = new Role(); role.setName(roleName);
 * roleMapper.insertSelective(role); }
 * 
 * @Override public List<Integer> getUserRoleList(String userId) { return
 * userRoleMapper.getRoleIdListByUserId(userId); }
 * 
 * }
 */